...
if (isset($_GET['mid'])) {
        ...
        DB::query('UPDATE messages SET readm=1 WHERE id=:mid', array (':mid'=>$_GET['mid']));
        ?>
...
foreach ($messages as $message) {
        ...
        if ($message['readm'] == 0) {
                echo "<a href='my-message.php?mid=".$message['id']."'><strong>".$m."</strong></a> sent by ".$message['username'].'<hr />';
        } else {
                echo "<a href='my-message.php?mid=".$message['id']."'>".$m."</a> sent by ".$message['username'].'<hr />';
        }
